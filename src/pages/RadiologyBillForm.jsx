...
import { patientAPI, radiologyAPI } from '../services/api';
...
const paymentModes = ['Cash', 'UPI', 'Debit Card', 'Credit Card', 'Insurance'];

export default function RadiologyBillForm() {
  ...
  useEffect(() => {
    ...
    radiologyAPI.getTests().then(res => setTests(res.data)).catch(() => {});
    if (isEdit) {
      radiologyAPI.getBillById(id).then(res => {
        const d = res.data;
        form.setFieldsValue({
        ...
        if (pay) {
          form.setFieldsValue({ paymentMode: pay.mode });
        }
  ...
  const addItem = () => {
    setItems([...items, { name: '', category: 'X-Ray', quantity: 1, price: 0, gstRate: 0, amount: 0, key: Date.now() }]);
  };
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (isEdit) { await radiologyAPI.updateBill(id, payload); message.success('Updated'); }
      else { await radiologyAPI.createBill(payload); message.success('Radiology bill created'); }
      navigate('/radiology/bills');
  ...
  const itemColumns = [
    ...
    {
      title: 'Investigation', dataIndex: 'name', key: 'name',
      render: (_, __, i) => (
        <Select
          value={items[i].test || undefined}
          onChange={(v) => handleTestSelect(i, v)}
          showSearch
          placeholder="Select X-Ray / CT / MRI / USG"
          allowClear
          style={{ width: '100%' }}
  ...
  return (
    <Card style={{ borderRadius: 10, maxWidth: 960, margin: '0 auto' }}>
      <Title level={4}>{isEdit ? 'Edit Radiology Bill' : 'New Radiology Bill'}</Title>
      <Form form={form} layout="vertical" onFinish={onFinish}
        ...
        <Divider>Bill Items <small style={{ fontWeight: 400 }}>(select from radiology catalog)</small></Divider>
        ...
        <Space>
          ...
          <Button onClick={() => navigate('/radiology/bills')} size="large">Cancel</Button>
        </Space>
      </Form>
    </Card>
  );

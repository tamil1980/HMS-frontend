...
export default function CollectionReport() {
  ...
  const fetchReport = async (start, end) => {
    ...
    try {
      ...
      const res = await invoiceAPI.collectionReport(params);
  ...
  const downloadReport = async (format) => {
    ...
    try {
      ...
      const res = await invoiceAPI.collectionReportFile(params);
      const ext = format === 'excel' ? 'xlsx' : 'pdf';
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `CollectionReport_${params.fromDate}_to_${params.toDate}.${ext}`);
  ...
  const columns = [
    ...
    { title: 'Invoice', dataIndex: 'invoiceId', key: 'id' },
    { title: 'Patient', dataIndex: ['patient', 'name'], key: 'patient', render: (t) => t || '-' },
    { title: 'Date', dataIndex: 'invoiceDate', key: 'date', render: (d) => d ? dayjs(d).format('DD/MM/YYYY') : '-' },
  ...
  return (
    <div>
      <Title level={4}>Collection Report</Title>
      ...
      {summary && (
        <Row gutter={16} style={{ marginBottom: 16 }}>
          <Col xs={12} sm={6}><Card size="small"><Statistic title="Total Invoices" value={summary.totalInvoices} /></Card></Col>

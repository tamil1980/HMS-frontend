...
export default function IPBills() {
  ...
  const fetchData = async (page = 1) => {
    ...
    try {
      ...
      const res = await ipAPI.getBills(q);
  ...
  useEffect(() => { fetchData(); }, [statusFilter, admission]);

  const downloadPDF = async (rec) => {
    try {
      const res = await ipAPI.getBillPDF(rec._id, true);
      downloadBlob(res, `IPBill_${rec.billId}_${rec.patient?.name || 'Unknown'}.pdf`);
    } catch { message.error('Download failed'); }
  };

  const print = async (rec) => {
    try { const res = await ipAPI.getBillPDF(rec._id); printPDF(res); }
    catch { message.error('Print failed'); }
  };

  const handleDelete = async (id) => {
    try { await ipAPI.removeBill(id); message.success('Deleted'); fetchData(); }
  ...
  const columns = [
    ...
    { title: 'Bill No', dataIndex: 'billId', key: 'id', render: (t, r) => <span>{t}{r.isAuto && <Tag color="geekblue" style={{ marginLeft: 6 }}>Room</Tag>}</span> },
    { title: 'Admission', dataIndex: ['admission', 'admissionId'], key: 'adm', render: (t, r) => t ? <span>{t}<br /><small>{r.admission?.ward || ''}/{r.admission?.bedNumber || ''}</small></span> : '-' },
    ...
    { title: 'Status', dataIndex: 'status', key: 'status', render: (s) => <Tag color={statusColors[s]}>{s}</Tag> },
    {
      ...
      render: (_, rec) => (
        <Space>
          <Tooltip title="View"><Button type="text" icon={<EyeOutlined />} onClick={() => navigate(`/ip/bills/${rec._id}/edit`)} /></Tooltip>
  ...
  return (
    <Card style={{ borderRadius: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        <Title level={4} style={{ margin: 0 }}>IP Bills{admission ? ` - Admission ${admission}` : ''}</Title>
        <Space>
          ...
          <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate(admission ? `/ip/bills/new?admission=${admission}` : '/ip/bills/new')}>New Bill</Button>
        </Space>
      </div>

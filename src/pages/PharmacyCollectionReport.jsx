...
export default function PharmacyCollectionReport() {
  ...
  const fetchReport = async (start, end) => {
    ...
    try {
      ...
      const res = await pharmacyAPI.collectionReport(params);
  ...
  const downloadReport = async (format) => {
    ...
    try {
      ...
      const res = await pharmacyAPI.collectionReportFile(params);
      const ext = format === 'excel' ? 'xlsx' : 'pdf';
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `PharmacyCollectionReport_${params.fromDate}_to_${params.toDate}.${ext}`);
  ...
  return (
    <div>
      <Title level={4}>Pharmacy Collection Report</Title>
      ...
      {summary && (
        <Row gutter={16} style={{ marginBottom: 16 }}>
          <Col xs={12} sm={4}><Card size="small"><Statistic title="Total Bills" value={summary.totalBills} /></Card></Col>
          <Col xs={12} sm={4}><Card size="small"><Statistic title="Gross Sales" value={summary.totalAmount} precision={2} prefix="₹" valueStyle={{ color: '#2563EB' }} /></Card></Col>
          <Col xs={12} sm={4}><Card size="small"><Statistic title="Returns" value={summary.totalReturns} precision={2} prefix="₹" valueStyle={{ color: '#DC2626' }} /></Card></Col>
          <Col xs={12} sm={4}><Card size="small"><Statistic title="Net Sales" value={summary.netAmount} precision={2} prefix="₹" valueStyle={{ color: '#059669' }} /></Card></Col>
          <Col xs={12} sm={4}><Card size="small"><Statistic title="Total Collected" value={summary.totalPaid} precision={2} prefix="₹" /></Card></Col>
          <Col xs={12} sm={4}><Card size="small"><Statistic title="Total Due" value={summary.totalDue} precision={2} prefix="₹" /></Card></Col>
        </Row>
      )}
      ...
      {returns.length > 0 && (
        <Card title={<span style={{ color: '#DC2626' }}>Sales Returns (deducted from net sales)</span>} style={{ borderRadius: 10 }}>
          <Table
            dataSource={returns}
            rowKey="_id"
            ...
            scroll={{ x: 800 }}
            columns={[
              { title: 'S.No', key: 'sno', width: 55, align: 'center', render: (t, r, i) => <div style={{ textAlign: 'center' }}>{i + 1}</div> },
              ...
              { title: 'Qty', key: 'qty', render: (_, r) => (r.items || []).reduce((s, it) => s + (Number(it.quantity) || 0), 0) },

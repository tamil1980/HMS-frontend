...
import { PlusOutlined, EyeOutlined, FilePdfOutlined, PrinterOutlined } from '@ant-design/icons';
...
export default function LabBills() {
  ...
  const fetchData = async (page = 1) => {
    ...
    try {
      ...
      const res = await labAPI.getBills(params);
  ...
  const downloadPDF = async (rec) => {
    try {
      const res = await labAPI.getBillPDF(rec._id, true);
      downloadBlob(res, `LabBill_${rec.billId}_${rec.patient?.name || 'Unknown'}.pdf`);
    } catch { message.error('Download failed'); }
  };

  const print = async (rec) => {
    try {
      const res = await labAPI.getBillPDF(rec._id);
      printPDF(res);
  ...
  const columns = [
    ...
    { title: 'Payment Mode', dataIndex: 'paymentMode', key: 'paymentMode', render: (m) => m ? <Tag>{m}</Tag> : '-' },
    { title: 'Status', dataIndex: 'status', key: 'status', render: (s) => <Tag color={statusColors[s]}>{s}</Tag> },
    {
      title: 'Actions', key: 'actions', width: 140,
      render: (_, rec) => (
        <Space>
          <Tooltip title="View"><Button type="text" icon={<EyeOutlined />} onClick={() => navigate(`/lab/bills/${rec._id}/edit`)} /></Tooltip>
  ...
  return (
    <Card style={{ borderRadius: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        <Title level={4} style={{ margin: 0 }}>Laboratory Bills</Title>
        <Space>
          ...
          <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/lab/bills/new')}>New Bill</Button>
        </Space>
      </div>

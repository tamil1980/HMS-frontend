...
import { consolidatedBillAPI } from '../services/api';
import { downloadBlob, printPDF } from '../utils/pdf';
import dayjs from 'dayjs';

const { Title } = Typography;

const sectionColors = { OP: 'blue', Lab: 'cyan', IP: 'red', Pharmacy: 'orange', Radiology: 'purple' };
...
export default function ConsolidatedBills() {
  ...
  const fetchData = async (page = 1) => {
    ...
    try {
      ...
      const res = await consolidatedBillAPI.getBills(params);
  ...
  const downloadPDF = async (rec) => {
    try {
      const res = await consolidatedBillAPI.getBillPDF(rec._id, true);
      downloadBlob(res, `ConsolidatedBill_${rec.billId}_${rec.patient?.name || 'Unknown'}.pdf`);
    } catch { message.error('Download failed'); }
  };

  const print = async (rec) => {
    try {
      const res = await consolidatedBillAPI.getBillPDF(rec._id);
      printPDF(res);
  ...
  const handleDelete = async (id) => {
    try {
      await consolidatedBillAPI.removeBill(id);
  ...
  const columns = [
    ...
    { title: 'Patient', dataIndex: ['patient', 'name'], key: 'patient', render: (t) => t || '-' },
    {
      ...
      render: (_, rec) => (rec.sections || []).map(s => <Tag key={s.type} color={sectionColors[s.type]}>{s.type}</Tag>),
    },
    ...
    { title: 'Status', dataIndex: 'status', key: 'status', render: (s) => <Tag color={statusColors[s]}>{s}</Tag> },
    {
      ...
      render: (_, rec) => (
        <Space>
          <Tooltip title="View"><Button type="text" icon={<EyeOutlined />} onClick={() => navigate(`/consolidated-bills/${rec._id}/edit`)} /></Tooltip>
          ...
          <Popconfirm title="Delete bill?" onConfirm={() => handleDelete(rec._id)}>
  ...
  return (
    <Card style={{ borderRadius: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        <Title level={4} style={{ margin: 0 }}>Consolidated Bills <small style={{ fontWeight: 400, fontSize: 13, color: '#666' }}>(OP + Lab + IP + Pharmacy + Radiology in one bill)</small></Title>
        <Space>
          ...
          <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/consolidated-bills/new')}>New Consolidated Bill</Button>
        </Space>
      </div>
